...
float ftm::dot(Vector v1, Vector v2)
{
	float res = 0;
...
Vector ftm::normalize(Vector vect)
{
	if (vect.get_dims() != 3)
...
Vector ftm::cross(Vector v1, Vector v2)
{
...
Matrix ftm::lookAt(Vector position, Vector target, Vector up)
{
...
Matrix ftm::perspective(float fovY, float aspect, float near, float far)
{
...
Matrix ftm::scale(Matrix original, Vector scales)
{
...
Matrix ftm::translate(Matrix original, Vector steps)
{
...
Matrix ftm::rotate(Matrix original, float angle, Vector axis)
{
	...
	for (size_t curr_row = 0; curr_row < 3; curr_row++)
	{
		...
		for (size_t curr_col = 0; curr_col < 4; curr_col++)
		{
			// for each column
			res_row.push_back(0);
			// loop through curr row matrix A (rot matrix)
			for (size_t b_idx = 0, col_count = 0; b_idx < 4; b_idx++, col_count += 4)
			{
				// for each col in mat A, access index [colCount + currRow]
				// in matrix B (og matrix)
